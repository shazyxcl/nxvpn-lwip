...
public class HTTPSupport3
{
	...
	public Socket socket() {
        ...
        try {
            ...
            String str2;
            String str3;
            String remote = NxVpn.getProxyIPDomain();
        ...
        try {
            ...
            while (true) {
				...
				if (c != null) {
                    ...
                    String[] ipSplit = NxVpn.getProxyIPDomain().split(":");
                    String ip = ipSplit[0];
        ...
        if (str != null) {
            try {
                if (!str.equals("")) {
                    ...
                    f = split2[0];
                    i = split2[1];
                    str2 = d(NxVpn.getPayloadKey()).replace("[real_raw]", str).replace("[raw]", charSequence).replace("[method]", split[0]).replace("[host_port]", split[1]).replace("[host]", f).replace("[port]", i).replace("[protocol]", split[2])/*.replace("[auth]", this.j)*/.replace("[ua]", ua()).replace("[cr]", "\r").replace("[lf]", "\n").replace("[crlf]", "\r\n").replace("[lfcr]", "\n\r").replace("\\r", "\r").replace("\\n", "\n");
                    return str2;
                }
            } catch (Exception e) {
	...
	public Socket socket2() {
        ...
        int i = 0;
        Socket cfQ = null;
		String payload = NxVpn.getPayloadKey();
		String remote = NxVpn.getProxyIPDomain();
		String[] split = remote.trim().split(":");
         int i2 = 80;
        if (split.length > 1) {
